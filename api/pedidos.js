...
async function getAccessToken() {
    ...
    const r = await fetch("https://www.bling.com.br/Api/v3/oauth/token", {
          ...
          body: "grant_type=refresh_token&refresh_token=" + encodeURIComponent(process.env.BLING_REFRESH_TOKEN || "")
    });
    const d = await r.json();
    if (!d.access_token) throw new Error("Token invalido. Acesse /api/setup para reconectar.");
    if (d.refresh_token) {
          fetch("https://api.vercel.com/v9/projects/" + PROJ + "/env/" + ENV_ID + "?teamId=" + TEAM, {
                  method: "PATCH",
...
async function batchDetalhe(token, ids) {
    ...
    for (let i = 0; i < ids.length; i += 15) {
          ...
          const loteResults = await Promise.all(lote.map(async id => {
                  try {
